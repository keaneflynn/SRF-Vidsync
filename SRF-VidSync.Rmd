...
Max_Fish_Tally_Dataset <- 
  ...
  mutate(index = as.numeric(str_extract(objects, "\\h\\d{1,2}"))) %>% 
  mutate(one_col = 1) %>% 
  select(one_col, date, BACI_date, site, sample_event, subsample, index, time) %>% 
  arrange(site, sample_event, subsample, time) %>% 
  group_by(sample_event, site, subsample, time) %>% 
...
SRF_Volume_Dataset <- 
  readr::read_csv(file = "Vidsync-data/BACI Fish Volume Data - SRF-Volumes.csv",
                  col_names = TRUE) %>% 
  select(stream, reach, date, event, pool, fish_species, subsample, index, fish_length_mm, num_fish_in_subsample, volume_occupied_all_cm3, volume_occupied_forage_cm3, points_in_sample, repeat_index)
SRF_Volume_Dataset
```
