...
Behavior_Types <-
  ...
  mutate(drift_forage = cumsum(if_else(grepl("^Drift_Forage", event), 1, 0))) %>%
  mutate(search_forage = cumsum(if_else(grepl("^Search_Forage", event), 1, 0))) %>%
  mutate(benthic_forage = cumsum(if_else(grepl("^Benthic_Forage", event), 1, 0))) %>%
  mutate(aggression = cumsum(if_else(grepl("Nip|Attack", event), 1, 0))) %>% 
  mutate(surface_strike = cumsum(if_else(grepl("^Surface_Strike", event), 1, 0))) %>% 
  mutate(movement = cumsum(if_else(grepl("^Movement", event), 1, 0))) %>%
  mutate(drift_forage_max = max(drift_forage)) %>% 
  mutate(search_forage_max = max(search_forage)) %>%
  mutate(benthic_forage_max = max(benthic_forage)) %>% 
  mutate(aggression_max = max(aggression)) %>% 
  mutate(surface_strike_max = max(surface_strike)) %>% 
  mutate(movement_max = max(movement)) %>%
  distinct(drift_forage_max, search_forage_max, benthic_forage_max, aggression_max, surface_strike_max, movement_max)
Behavior_Types

Forage_Types <- Behavior_Types %>%
  select(site, sample_event, drift_forage_max, search_forage_max, benthic_forage_max)
Forage_Types
```
####Post-Augmentation
```{r}
