...
DO_Dataset <- 
  ...
  group_by(Date) %>% 
  na.omit() %>% 
  summarize(DO_mg_L = mean(DO_mg_L))
DO_Dataset$Date <- base::as.Date(DO_Dataset$Date, "%m/%d/%Y")
...
Hydrology_Dataset <- RCT_Dataset %>% 
  ...
  left_join(DO_Dataset, by = c("SampleDate" = "Date")) %>% 
...
RCT_DO_Graph <- 
  ggplot(Hydrology_Dataset) +
  geom_smooth(aes(x = RCT_RR, y = DO_mg_L, color = "Site 18.5")) +
  geom_smooth(aes(x = RCT_BB, y = DO_mg_L, color = "Site 18.6")) +
  geom_smooth(aes(x = RCT_HT, y = DO_mg_L, color = "Site 18.8")) +
  scale_x_reverse() +
  labs(color = "Study Site") +
  xlab("Riffle Crest Thalweg (RCT) Depth in cm") +
  ylab("Dissolve Oxygen (mg/L)") +
  ggtitle("Effect of RCT Depth on Dissolved Oxygen Content") +
